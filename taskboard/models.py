...
class Project(BaseModel):
    ...
    end_date = models.DateField(_('End Date'),help_text='end date', null=True, blank=True)
    priority = models.CharField(max_length=128, choices=PRIORITY_CHOICES, default='Normal', null=True, blank=True)
    project_leader = models.ForeignKey("employee.Employee", on_delete=models.CASCADE, limit_choices_to={'is_deleted': False}, related_name='project_leader', null=True, blank=True)
    team = models.ManyToManyField("employee.Employee", verbose_name=_("Team"), related_name='project_team')
